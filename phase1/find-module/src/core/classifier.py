...
def calculate_confidence(sorted_scores: list[tuple[str, float]]) -> float:
    ...
    # Nếu điểm cao nhất chưa đạt ngưỡng tối thiểu, trả về 0 luôn
    if top_score < UNKNOWN_THRESHOLD:
        return 0.0

    # Tính tổng điểm của Top 2 để làm mẫu số
    total_top2 = top_score + second_score
    
    # Đề phòng trường hợp cả 2 đều bằng 0 (dù logic ở trên đã chặn top_score < 8.0)
    if total_top2 == 0:
        return 0.0

    # Tỷ lệ % điểm của Top 1 so với tổng Top 2
    confidence = top_score / total_top2
...
def classify_issue(issue: dict[str, Any]) -> dict[str, Any]:
    ...
    if top_score < UNKNOWN_THRESHOLD:
        ...
        if gap < LOW_CONFIDENCE_GAP:
            review_status = "manual_review" # Phải có người vào kiểm tra tay
        elif gap < HIGH_CONFIDENCE_GAP:
            review_status = "review_recommended" # Nên lướt qua xem lại, dù hệ thống đã có dự đoán
        else:
            review_status = "auto_assign" # Tự tin tuyệt đối, tự động gán luôn cho PIC

    explain_reasons = sorted(
